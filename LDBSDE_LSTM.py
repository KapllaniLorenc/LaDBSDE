...
class ldbsdeLSTM(tf.keras.Model):
    def __init__(self, eqn_config, net_config, bsde):
        super().__init__()
        self.eqn_config = eqn_config
        self.net_config = net_config
        ...
        self.c_m1 = tf.Variable(tf.random.normal([1, eta[0]], stddev=np.sqrt(2) / np.sqrt(1 + eta[0]), dtype=tf.float32))
    ...
    def call(self, inputs, training):
        ...
        c = all_one_vec * self.c_m1
        for n in range(0, self.bsde.N+1):
            t_n_tf = t[n]*all_one_vec
            y_n, z_n, h, c = self.DNN_y_z((t_n_tf, X[:, :, n], h, c))
            Yhat.append(y_n)
            Zhat.append(z_n)
        return Yhat, Zhat
...
class FeedForwardNet_y_z(tf.keras.Model):
    def __init__(self, eqn_config, net_config, bsde):
        ...
        self.bsde = bsde
        eta = net_config["eta"]        
        self.dense_layers_cX = tf.keras.layers.Dense(eta[0], use_bias=False, activation=None)
        self.dense_layers_uX = tf.keras.layers.Dense(eta[0], use_bias=False, activation=None)
        self.dense_layers_fX = tf.keras.layers.Dense(eta[0], use_bias=False, activation=None)
        self.dense_layers_oX = tf.keras.layers.Dense(eta[0], use_bias=False, activation=None)
        
        self.dense_layers_ch = tf.keras.layers.Dense(eta[0], use_bias=True, activation=None)
        self.dense_layers_uh = tf.keras.layers.Dense(eta[0], use_bias=True, activation=None)
        self.dense_layers_fh = tf.keras.layers.Dense(eta[0], use_bias=True, activation=None)
        self.dense_layers_oh = tf.keras.layers.Dense(eta[0], use_bias=True, activation=None)
    ...
    def call(self, x):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            c_tilde = tf.tanh(tf.add(self.dense_layers_ch(h), self.dense_layers_cX(inp)))
            gam_u = tf.sigmoid(tf.add(self.dense_layers_uh(h), self.dense_layers_uX(inp)))
            gam_f = tf.sigmoid(tf.add(self.dense_layers_fh(h), self.dense_layers_fX(inp)))
            gam_o = tf.sigmoid(tf.add(self.dense_layers_oh(h), self.dense_layers_oX(inp)))            
            c = tf.add(tf.multiply(gam_u, c_tilde), tf.multiply(gam_f, c))
            h = tf.multiply(gam_o, tf.tanh(c))
